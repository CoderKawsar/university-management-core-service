...
const addToDB = async (
  ...
  if (isAnySemesterRegistrationOngoing) {
    throw new ApiError(
      ...
      `There is already an ${isAnySemesterRegistrationOngoing.status} Registration`
    );
  }
...
const getByIdFromDB = async (
  ...
  const result = prisma.semesterRegistration.findUnique({
    where: {
      id,
    },
    include: { academicSemester: true },
...
const updateOneInDB = async (
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'Status can only be from UPCOMING to ONGOING'
    );
  }
  ...
  if (
    ...
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'Status can only be from ONGOING to ENDED'
    );
  }
